...
(function($) {
	...
	$.omWidget("om.omPanel" , {
		 ...
		 */
	 	...
	 	//	console.log(options.todo+'resize'+options.height);
        ...
        _makeColsResizable:function(){
            ...
            $('th[axis^="col"] div',self.thead).omResizable({
                ...
                resize: function(ui , event) {
                    ...
                    var toolbarDiv=$('.toolbarDiv',grid);
                    var bDivHeight=grid.height()-($titleDiv.is(":hidden")?0:$titleDiv.outerHeight(true))-(toolbarDiv.length>0?toolbarDiv.outerHeight(true):0)-hDiv.outerHeight(true)-(self.pDiv.is(":hidden")?0:self.pDiv.outerHeight(true))
...
/*            .parent().unbind().bind('mouseleave',function(){
        ...
        show: function() {
            ...
            this.con.css('top', $container.outerHeight()-1);
        },
	...
	$.extend($.om.omCalendar, {
		Page: function(config, father) {
            ...
            this.nav_html = [
                ...
                '</p>',
                '<p>',
                '<button class="ok btn btn-mini" type="button">',
                i18n.ok,
                '</button>&nbsp;<button class="cancel btn btn-mini" type="button">',
                i18n.cancel,
                '</button>',
                '</p>'
            ].join("");
...
;(function($) {
	...
	$.omWidget('om.omCombo', {
        ...
        _create:function(){
            ...
            var span = $('<span class="om-combo om-widget om-state-default"></span>').css('position','relative').insertAfter(inputEl).wrapInner(inputEl);
...
/*            if(this.options['pager']){
        ...
        setData:function(param){
            var self = this, inputEl = self.textInput, valueEl = self.valueEl;
            self.options.value = '';
            valueEl.val('');
            inputEl.val('');
...
/*        _dropListToggle:function(){
        ...
        _bindEvent:function(){
            ...
            valueEl = self.valueEl, dropList = self.dropList,
...
/*                if(options.pager){
        ...
        _initDropList:function(){
        	var self = this, options = self.options, 
        	    inputEl = self.textInput, valueInput = self.valueEl,
            ...
            if (valueInput.val() !== undefined && valueInput.val() !== '') {
            	var nowValue = valueInput.val();
        ...
        _loadData:function(records){
            ...
            if (options.listProvider) {
                ...
                var optionField = options.optionField,valueField=options.valueField;
                var innerHtml = '';
                var self = this;
                $(records).each(function(index){
                	var row='<div class="om-combo-list-row">';
                	if (typeof optionField === 'string') {
                		row+=this[options.optionField];
                	}else{
                		row+=options.optionField(this, index);
                	}
                	row+='</div>';
                	innerHtml+=row;
...
/*        _wrapText:function(text) {
        ...
        _setValue:function(value){
            var input = this.textInput, valueEl = this.valueEl;
           ...
           // valueEl.removeAttr('value').attr('value',value);
        ...
        _backfill:function(source){
            ...
            var self = this, valueEl = self.valueEl,
...
/*                if(options.pager){
        ...
        _doFilter:function(){
        	var self = this, inputEl = self.textInput, valueEl = self.valueEl, options = self.options;
...
;(function($) {
    ...
    $.omWidget('om.omItemSelector', {
        options: /** @lends omItemSelector#*/{
            ...
            lazyLoad : false,
            /**
            * 是否延迟加载数据,插件初始化不加载数据可以选择之后使用remoteData方法来重新加载
            * @type Boolean
            * @default false
            * @example
            * */
        ...
        _create:function(){
            this.element.addClass('om-itemselector om-widget').html('<div class="om-itemselector-leftpanel om-itemselector-panel"></div><div class="om-itemselector-toolbar om-itemselector-panel"></div><div class="om-itemselector-rightpanel om-itemselector-panel"></div>');
            var panels=$('.om-itemselector-panel',this.element);
           // var thead = $('<thead></thead>');
            var cell = $('<h2></h2>').attr({axis:'checkboxCol',align:'center'})
            .append($('<div class="header"><span class="checkbox"/><span class="om-itemselector-title"/></div>'));
           // $('<tr></tr>').append(cell).appendTo(thead);
            this.leftPanel = panels.eq(0).append(cell)
            .append('<div class="om-itemselector-items"><dl></dl></div>');
            this.toolbar=$('<div></div>').appendTo(panels.eq(1));
            this.rightPanel=panels.eq(2).append(cell.clone())
                .append('<div class="om-itemselector-items"><dl></dl></div>');
        },
        _init:function(){
            var op=this.options,
                dataSource=op.dataSource,elementWidth=parseInt(op.width),elementHeight=parseInt(op.height);

            var panelWidth=(elementWidth-60)/ 2,panelHeight=elementHeight-2;
            this.leftPanel.css({width:panelWidth+'px',height:panelHeight+'px'}).find(".om-itemselector-title").html($.om.lang._get(op,"omItemSelector","availableTitle"));
            this.rightPanel.css({width:panelWidth+'px',height:panelHeight+'px'}).find(".om-itemselector-title").html($.om.lang._get(op,"omItemSelector","selectedTitle"));
            this.toolbar.css({position:'absolute',top:'35%',left:'25%'}).parent().css({height:panelHeight+'px',position:'relative'});
            this.element.css({height:op.height});
            this._buildToolbar();
            ...
            if(typeof dataSource ==='string'){
                var self=this;
                if(op.lazyLoad){
                    self.remoteUrl=dataSource;
                    self._buildValueList();
                }else{
                $.ajax({
                    url: dataSource,
                    type: 'GET',
        ...
        _resizePanel:function(){
            ...
            leftItemsContainer.children('dl').outerHeight(H);
            rightItemsContainer.children('dl').outerHeight(H);
...
/*        _buildOptionList:function(){
        ...
        _buildValueList:function(){
            ...
            $.ajax({
                ...
                success: function(data, textStatus){
                    ...
                    //self._refreshPageButton(self.leftPanel);
                    var valueData=[];
                    if($.isArray(data) && jQuery.isArray(value)){
                        $.each(data,function(index,_data){
                            if(inArray(_data,value)){//在value中，要放到右边
                            	valueData.push(data[index]);
                                rightHtml+=buildHtml(index,_data);
                            }
                        });
                    }
                    $('.om-itemselector-items>dl',self.rightPanel).html(rightHtml);
                    self.element.data('valueData',valueData);
                    var items = $('.om-itemselector-items'),itemdtH =$('>dl>dt',items).outerHeight(),
                        h =items.outerHeight(),offset = itemdtH - h%itemdtH;
                    items.outerHeight(h+offset);
                },
        ...
        _buildList:function(){
            var op=this.options;
                ...
                rightHtml='',
                valueData=[],
                storeData=this.element.data('valueData'),
                // {text:'abc',value:2}的value是否在value:[0,2,4]这样的数组中
                inArray=function(data,valueArr){
                    for(var i=0,len=valueArr.length;i<len;i++){
                        if(data.value==valueArr[i]){
            ...
            if($.isArray(dataSource) && jQuery.isArray(value)){
                if(storeData != undefined){
                    var storeDataValues=[],newData=[];
                    $.each(storeData,function(){
                        storeDataValues.push(this.value);
                    });
                    for(var j= 0;j<dataSource.length;j++){
                        if(inArray(dataSource[j],storeDataValues)){
                        	continue;
                        }
                        newData.push(dataSource[j]);
                    }
                    $.merge(newData,storeData);
                   // $.unique(dataSource);
                }
                $.each(dataSource,function(index,data){
            ...
            this.element.data('valueData',valueData);
            $('.om-itemselector-items>dl',this.leftPanel).html(leftHtml);
...
/*        _refreshPageButton: function(panel){
            ...
            this.element.css({height:parseInt(this.options.height)+2});
        },*/
/*        _getPageItems:function(panel, $dt){
        ...
        _sortItems:function(panel){
            var sortItems=$('.om-itemselector-items>dl>dt',panel),
                fmt=this.options.clientFormatter,
                sortList=[],
                sortedList,
                sortedHtml='';
            ...
            sortItems.each(function(){
                sortList.push({'_index':$(this).attr('_index'),'text':$(this).html()});
            });
            sortedList=sortList.sort(function(a,b){
                return a._index-b._index;
            });
            $.each(sortedList,function(index,data){
                sortedHtml+='<dt _index="'+data._index+'">'+data.text+'</dt>';
            });
            $('.om-itemselector-items>dl',panel).html(sortedHtml);
        },
	...
	//            omtabs.children(':first').css('width',options.width);
	        ...
	        if (options.closable && options.tabMenu && $.fn.omMenu) {
	        	var tabId = $self.attr('id'),
	        		menuDatasource=[{id:tabId+'_001',label:'关闭'},
	                              {id:tabId+'_002',label:'关闭其它'}
	                             ];
	            if(options.staticTab.length === 0){
	            	menuDatasource.push({id:tabId+'_003',label:'关闭所有'});
	            }
		...
		_renderHeader : function() {
	        ...
	        $lis.each(function(n){
	            ...
	            if (options.closable===true || ($.isArray(options.closable) && -1 !== $.inArray(n,options.closable))) {
	            	if ($.inArray(n,options.staticTab) === -1){
...
/*	        var contentPos = {},
    	...
    	_close : function(index) {
	        ...
	        if(-1 !== $.inArray(index,options.staticTab)){
	        	return false;
	        }
 ...
 *  om-resizable.js
 */
define('vendor/omui/om-window',function(require,exports,module){
	var $ = require('jquery');
(function($){
	var uiDialogClasses =
		'om-dialog ' +
...
*/
		...
		if(options.maximizable){
			self._makeMaximizable();
		}
		if(options.minimizable){
			self._makeMinimizable();
		}
//		console.log("build window");
		uiDialogTitlebar.children('.om-dialog-titlebar-minimum').hide();
		//self._createButtons(options.buttons);
		self._isOpen = false;
	...
	open: function() {
		...
		//self._position(options.position);
		uiDialogTitlebar.children('.om-dialog-titlebar-minimum').hide();
		uiDialogTitlebar.children('.om-dialog-titlebar-maximum').show();
		uiDialog.show(options.show);
		self.moveToTop(true);
	...
	_makeMaximizable:function(){
		var	self=this,options = self.options,
			uiDialog=self.uiDialog,
			uiDialogTitlebar=self.uiDialogTitlebar,
			uiDialogTitlebarMaximum = $('<a href="#"></a>')
				.addClass(
					'om-dialog-titlebar-maximum ' +
					'om-corner-tr'
				)
				.attr('role', 'button')
				.hover(
					function() {
						uiDialogTitlebarMaximum.addClass('om-state-hover');
					},
					function() {
						uiDialogTitlebarMaximum.removeClass('om-state-hover');
					}
				)
				.focus(function() {
					uiDialogTitlebarMaximum.addClass('om-state-focus');
				})
				.blur(function() {
					uiDialogTitlebarMaximum.removeClass('om-state-focus');
				})
				.appendTo(uiDialogTitlebar),

			uiDialogTitlebarMaximumText = (self.uiDialogTitlebarMaximumText = $('<span></span>'))
				.addClass('om-icon-closethick')
				.text(options.maximumText)
				.appendTo(uiDialogTitlebarMaximum);
		uiDialogTitlebarMaximum.unbind().bind('click',function(){
			var position={
				my: 'top',
				at: 'left',
				collision: 'fit'
			},windowWidth=$(document).width()-5,windowHeight=$(document).height()-5;
			$.extend(options.position,position);
			uiDialog.css({
				width:windowWidth,
				height:windowHeight,
				top:0,
			...
			uiDialog.children('.om-dialog-content').css({
				width:windowWidth,
				height:windowHeight-uiDialogTitlebar.outerHeight()
			});
			uiDialogTitlebar.children('.om-dialog-titlebar-maximum').hide();
			uiDialogTitlebar.children('.om-dialog-titlebar-minimum').show();
		});
	...
	_makeMinimizable:function(){
		var	self=this,options = self.options,
			uiDialog=self.uiDialog,
		    uiDialogTitlebar=this.element.parent().children('.om-dialog-titlebar');
			uiDialogTitlebarMinimum = $('<a href="#"></a>')
				.addClass(
					'om-dialog-titlebar-minimum ' +
					'om-corner-tr'
				)
				.attr('role', 'button')
				.hover(
					function() {
						uiDialogTitlebarMinimum.addClass('om-state-hover');
					},
					function() {
						uiDialogTitlebarMinimum.removeClass('om-state-hover');
					}
				)
				.focus(function() {
					uiDialogTitlebarMinimum.addClass('om-state-focus');
				})
				.blur(function() {
					uiDialogTitlebarMinimum.removeClass('om-state-focus');
				})
				.appendTo(uiDialogTitlebar),

			uiDialogTitlebarMinimumText = (self.uiDialogTitlebarMinimumText = $('<span></span>'))
				.addClass('om-icon-closethick')
				.text(options.minimumText)
				.appendTo(uiDialogTitlebarMinimum);
		uiDialogTitlebarMinimum.unbind().bind('click',function(){
			var position={
				my: 'center',
				at: 'center',
			}
